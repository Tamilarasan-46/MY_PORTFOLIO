    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 id="education-heading" className="text-display-sm lg:text-display-md font-bold mb-5">
            Learning <span className="text-brand">journey</span>
          </h2>
          <p className="text-lg text-fg-subtle">
            Formal grounding in information technology, kept current with focused certifications.
          </p>
        </header>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-5">
                    ...
                    >
                      <item.icon className={`w-7 h-7 ${a.text}`} aria-hidden="true" />
                    </div>

                    <div className="min-w-0">
                      <h3 className="text-xl font-semibold text-fg mb-1.5">{item.degree}</h3>
                      <p className="text-beam font-medium mb-4">{item.institution}</p>
          ...
          <div className="lg:col-span-5 space-y-6">
            {certifications.map((cert, index) => {
              ...
              return (
                <article
                  key={cert.title}
                  className={`surface-card surface-card-hover ${a.borderHover} p-6 transition-all duration-700 ease-spring ${
                    isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
                  }`}
                ...
                >
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    >
                      <cert.icon className={`w-5 h-5 ${a.text}`} aria-hidden="true" />
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-semibold text-fg">{cert.title}</h3>
